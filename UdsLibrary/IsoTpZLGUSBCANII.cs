                ...
                if (array.Length > 0)
                {
                    ...
                    foreach (var q in query)
                    {
                        ...
                        Console.WriteLine($"{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff")} " +
                            $"{can.CanPara.deviceIndex} CanId:0x{GetId(q.frame.can_id).ToString("X")},通道:{hdl.Channel} 接收:{BitConverter.ToString(q.frame.data)}");
                        LogInfo?.Invoke($"USBCANII CanId:0x{GetId(q.frame.can_id).ToString("X")}" +
